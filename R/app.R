...
# Fonction pour mettre à jour la matrice du joueur
update_player_matrix <- function(i, j) {
  current_joueur <- joueur()
  current_value <- current_joueur[i, j]
...
ui <- fluidPage(
  ...
  headerPanel('Picross'),
  tags$a("Page GitHub", href = "https://github.com/Guillaume-BR/picross"),
...
# Logique serveur
server <- function(input, output, session) {  
  # Cliquez automatiquement sur le bouton générer lors du chargement
  observe({
    shinyjs::runjs('$("#btn_generer").click();')
  })
...
shinyApp(ui, server)
>>>>>>> 6f4cdb4d20f658a7ab7b3fc145a05179f88cb0c2:R/app.R
